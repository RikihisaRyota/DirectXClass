...
void Player::BehaviorRootUpdate() {
	if (!IsHit_) {
		...
		hit_Count_--;
		worldTransform_.at(0).translation_ += hit_Move_;
		if (hit_Count_ <= 0) {
			IsHit_ = false;
			worldTransform_Motion_.at(0).rotation_.x = 0.0f;
		}
	}
}
...
void Player::OnCollision(const OBB& obb, uint32_t type) {
	if (type == static_cast<uint32_t>(Collider::Type::PlayerToEnemy)) {
	...
	else if (type == static_cast<uint32_t>(Collider::Type::PlayerToEnemyAttack)) {
		...
		if (enemyAttack_->GetAttack()) {
			...
			if (overlapX < overlapZ) {
				if (distance.x < 0.0f) {
					move = Vector3{ overlapX, 0, 0 } *0.05f;
				}
				else {
					move = Vector3{ -overlapX, 0, 0 } *0.05f;
				}
				...
				if (distance.z < 0.0f) {
					move = Vector3{ 0, 0, overlapZ } *0.05f;
				}
				else {
					move = Vector3{ 0, 0, -overlapZ } *0.05f;
				}
			...
			hit_Move_ = move;
			PlayerRotate(obb.center_ - worldTransform_.at(0).translation_);
			worldTransform_Motion_.at(0).rotation_.x = DegToRad(-15.0f);
			hit_Count_ = hit_Count_Max;
			IsHit_ = true;
			// 転送
			BaseCharacter::Update();
			HitBoxUpdate();
