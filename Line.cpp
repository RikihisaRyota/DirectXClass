...
//	assert(line);
//	// パイプライン初期化
//	line->lineGraphicsPipline_ = std::make_unique<LineGraphicsPipline>();
//	line->lineGraphicsPipline_->InitializeGraphicsPipeline();
//	// 初期化
//	line->Initialize();
//	return line;
//}
...
void Line::BasicDraw(
	...
	vertices_.at(0) = Vertex({ v1.x,v1.y,v1.z,1.0f }, {color});
	vertices_.at(1) = Vertex({ v2.x,v2.y,v2.z,1.0f }, {color});
	Vertex* vertMap = nullptr;
	result = vertBuff_->Map(0, nullptr, reinterpret_cast<void**>(&vertMap));
	if (SUCCEEDED(result)) {
		std::copy(vertices_.begin(), vertices_.end(), vertMap);
