...
// 静的メンバ変数の実体化
ID3D12Device* Plate::sDevice = nullptr;
Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> Plate::cmdList_;

void Plate::SetDevice(ID3D12Device* device) {
	// nullptrチェック
	assert(device);
...
void Plate::PreDraw(ID3D12GraphicsCommandList* cmdList) {
...
Plate* Plate::Create(uint32_t IsLighting, bool IsToon) {
	// Planeのインスタンスを生成
	Plate* sptite = new Plate();
	assert(sptite);
	// パイプライン初期化
...
void Plate::Draw(const WorldTransform& worldTransform, const ViewProjection& viewProjection, uint32_t textureHadle) {
	if (IsToon_) {
...
void Plate::SetToon(uint32_t IsToon) {
	IsToon_ = IsToon;
}

void Plate::SetDirectionalLight(const cDirectionalLight& DirectionalLight) {
...
void Plate::SetMaterial(const cMaterial& material) {
...
void Plate::BasicDraw(const WorldTransform& worldTransform, const ViewProjection& viewProjection, uint32_t textureHadle) {
	// ルートシグネチャの設定
...
void Plate::ToonDraw(const WorldTransform& worldTransform, const ViewProjection& viewProjection, uint32_t textureHadle) {
	// ルートシグネチャの設定
...
ComPtr<ID3D12Resource> Plate::CreateBuffer(UINT size) {
	HRESULT result = S_FALSE;
	// ヒーププロパティ
